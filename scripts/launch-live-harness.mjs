...
import { ensurePrivateDirectory, writePrivateFile } from '../src/core/private-files.js';
import { DEFAULT_LIVE_PROFILE_DIR } from '../src/core/paths.js';
...
function printSession(session) {
  ...
  console.log('  MyChart:       configured portal');
  console.log('');
...
export async function launchLiveHarness({
  ...
  const { profileDir } = resolveHarnessPaths({
    rootDir,
    profileDir: process.env.AWESOME_MYCHART_PROFILE_DIR || DEFAULT_LIVE_PROFILE_DIR,
  });
  ...
  await writePrivateFile(getSessionFile(profileDir), `${JSON.stringify(session, null, 2)}\n`, 'utf8');
