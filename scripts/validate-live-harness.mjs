...
export async function validateLiveHarness({
  ...
  const endpoint = await resolveValidationEndpoint({ port, profileDir });
...
async function resolveValidationEndpoint({ port, profileDir } = {}) {
  if (profileDir) {
    ...
    try {
      ...
      // A profile may not have a session file yet; use the configured CDP port.
    }
  }
  return createLiveHarnessEndpoint(port);
}
...
function printValidation(result) {
  ...
  if (result.patientContext) console.log('Patient context: detected');
