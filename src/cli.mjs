...
import { formatSyntheticDemo } from './core/demo.js';

function usage() {
  return `Usage:
  mychart-cli demo
  mychart-cli browser start [--visible|--headless] [--profile PATH] [--data-dir PATH] [--url URL]
  mychart-cli browser check
  mychart-cli browser validate
  mychart-cli browser ensure [--headless|--visible] [--validate] [--wait] [--timeout-seconds N] [--profile PATH] [--data-dir PATH] [--url URL]
  mychart-cli sync [--login] [--categories visits,test-results] [--require-active-patient TEXT] [--exhaustive] [--max-broad-pages N] [--seed-url URL] [--force] [--max-pages N] [--max-records N] [--switch-patient TEXT]
  mychart-cli records list [--patient-label-exact TEXT] [--category TEXT] [--query TEXT] [--start-date YYYY-MM-DD] [--end-date YYYY-MM-DD] [--json] [--store PATH] [--data-dir PATH]
  mychart-cli records show --id ID [--json] [--store PATH] [--data-dir PATH]
  mychart-cli export markdown [--sync] [--latest-day|--since-last-pull --pull-state PATH|--days N|--start-date YYYY-MM-DD --end-date YYYY-MM-DD|--all] [--output PATH|--output-dir PATH] [--json-summary] [--store PATH] [--data-dir PATH]
  mychart-cli export jsonl [--sync] [--latest-day|--since-last-pull --pull-state PATH|--days N|--start-date YYYY-MM-DD --end-date YYYY-MM-DD|--all] [--output PATH|--output-dir PATH] [--json-summary] [--store PATH] [--data-dir PATH]
  mychart-cli export inspect FILE [--json]
`;
}
...
function applyRuntimePathDefaults(options = {}) {
  if (!options.dataDir) return options;
  const resolved = resolveMyChartPaths({ dataDir: options.dataDir });
  process.env.MYCHART_CLI_HOME = resolved.dataHome;
  options.profileDir ||= resolved.profileDir;
  options.storePath ||= resolved.storePath;
  options.attachmentsDir ||= resolved.attachmentsDir;
  options.envFile ||= resolved.envPath;
  return options;
}
...
async function commandBrowser(subcommand, options) {
  ...
  if (subcommand === 'check' || subcommand === 'validate') {
    ...
    const validation = await validateLiveHarness({
      requireAuth,
...
function printBrowserStatus(validation, { requireAuth = false } = {}) {
  console.log(JSON.stringify({
    ...
    patientContext: validation.patientContext ? 'detected' : null,
    needsMfa: validation.needsMfa,
    endpoint: validation.endpoint,
    mychartTabCount: validation.mychartTabs.length,
...
function sanitizeStatusText(value = '') {
  return String(value || '').replace(/https?:\/\/\S+/gi, '[portal-url]');
}
...
async function ensureBrowserHarness(options = {}) {
  ...
  let launched = null;
  let validation = await validateLiveHarness({
    requireAuth,
    profileDir: options.profileDir,
  }).catch((error) => ({
    ok: false,
  ...
  do {
    validation = await validateLiveHarness({
      requireAuth,
      profileDir: options.profileDir,
    }).catch((error) => ({
      ok: false,
...
async function commandSync(options) {
  ...
  await withSession(options, async ({ browser, page, session }) => {
    if (options.login) {
      await loadEnvironmentFile({ envPath: options.envFile || DEFAULT_ENV_PATH });
    ...
    const status = await triggerRawDeepSync(page, {
      ...
      attachmentsDir: options.attachmentsDir || DEFAULT_ATTACHMENTS_DIR,
      timeoutSeconds: options.timeoutSeconds || DEFAULT_TIMEOUT_SECONDS,
      onProgress: (progress) => {
        console.error(
...
async function commandExport(subcommand, options) {
  ...
  if (options.jsonSummary) {
    console.log(JSON.stringify(sanitizeJsonSummary(summary), null, 2));
    return;
  }
  ...
  if (summary.safeSummary) printSafeSummary(summary.safeSummary);
}

function sanitizeJsonSummary(summary = {}) {
  return {
    ...summary,
    outputPath: summary.outputPath ? '[private output path]' : '',
    pullState: summary.pullState
      ? {
          ...summary.pullState,
          ...(summary.pullState.path ? { path: '[private pull-state path]' } : {}),
        }
      : null,
  };
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (command === 'sync') {
    const syncOptions = applyRuntimePathDefaults(parseOptions([subcommand, ...rest].filter(Boolean)));
    if (syncOptions.help) {
      process.stdout.write(usage());
      return;
    }
  ...
  if (command === 'demo') {
    const demoOptions = parseOptions([subcommand, ...rest].filter(Boolean));
    if (demoOptions.help) {
      process.stdout.write(usage());
      return;
    }
    process.stdout.write(`${formatSyntheticDemo()}\n`);
    return;
  }
  const options = applyRuntimePathDefaults(parseOptions(rest));
  if (options.help) {
    process.stdout.write(usage());
    return;
  }
...
const invokedUrl = process.argv[1] ? pathToFileURL(realPathOrOriginal(process.argv[1])).href : null;
