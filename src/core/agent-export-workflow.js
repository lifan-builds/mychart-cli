...
export async function runAgentExportWorkflow(options = {}) {
    ...
    try {
      if (options.login) {
        await loadEnvironmentFile({
          envPath: options.envFile || DEFAULT_ENV_PATH,
        });
  ...
  return {
    ...
    latestClinicalDate,
    filters: sanitizeSummaryFilters(filters),
    stored: {
    ...
    sourceHostCounts: redactSourceHostCounts(exportDownload.exportedCards, filtered.recordsById),
    ...
    safeSummary: createSafeSummary(exportDownload.exportedCards, filtered.recordsById),
...
function redactSourceHostCounts(cards = [], recordsById = new Map()) {
  const sourceCount = cards.filter((card) => {
    const record = recordsById.get(card.id) || {};
    const sourceUrl = card.sourceUrl || record.sourceUrl || '';
    try {
      return Boolean(new URL(sourceUrl).host);
    } catch {
      return Boolean(sourceUrl);
    }
  }).length;
  return sourceCount ? { redacted: sourceCount } : {};
}

function createSafeSummary(cards = [], recordsById = new Map()) {
...
function sanitizeSummaryFilters(filters = {}) {
  return {
    ...
    hasPatientFilter: Boolean(filters.patientKey || filters.patientLabel),
