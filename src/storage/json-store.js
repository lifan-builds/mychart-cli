...
import { applyPrivateMode, ensurePrivateDirectory, PRIVATE_FILE_MODE, writePrivateFile } from '../core/private-files.js';
...
export class JsonMedicalStore {
  ...
  async writeStore(store) {
    ...
    // The caller may intentionally place --store inside an existing shared
    // directory. Create missing parents privately, but never narrow an
    // existing caller-owned parent as a side effect of writing the store.
    await ensurePrivateDirectory(path.dirname(this.storePath), { hardenExisting: false });
    const tmpPath = `${this.storePath}.${process.pid}.${Date.now()}.tmp`;
    await writePrivateFile(tmpPath, `${JSON.stringify(nextStore, null, 2)}\n`, 'utf8');
    await rename(tmpPath, this.storePath);
    await applyPrivateMode(this.storePath, PRIVATE_FILE_MODE);
    return nextStore;
  }
