...
test('runAgentExportWorkflow writes export and returns safe machine-readable summary', async () => {
  ...
  assert.deepEqual(summary.sourceHostCounts, { redacted: 2 });
  assert.deepEqual(summary.safeSummary, {
    latestClinicalDate: '2026-06-10',
    categories: [
      { category: 'test-results', count: 1 },
      { category: 'visits', count: 1 },
    ],
  });
  assert.equal(JSON.stringify(summary).includes('Demo Child'), false);
  assert.equal(JSON.stringify(summary).includes('mychart.example.test'), false);
