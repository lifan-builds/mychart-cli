...
test('CLI export emits safe JSON summary and protects state without scoped freshness', async () => {
  ...
  const identifyingOutputPath = path.join(outputDir, 'Demo Child records.jsonl');
  ...
  assert.equal(summary.outputPath, '[private output path]');
  ...
  assert.equal(JSON.stringify(summary).includes(identifyingOutputPath), false);

  const exportText = await readFile(identifyingOutputPath, 'utf8');
